...
CALL spCancelarPedido(5,3,@msg); # por si acaso este es el mismo que se creo en el SP1
...
DETERMINISTIC # Utiliza cosas de la base de datos. Toca usar un método / función numérico.
BEGIN
	declare v_porcentaje decimal (5,2);
    declare v_total decimal(12,2);
    
    # Definir el porcentaje de descuento según la cantidad comprada. Entre mayor sea la cantidad, mayor el descuento.
    # Se parece a un Switch:
    SET v_porcentaje = case # El THEN le asigna el valor a la variable elegida
...
p.nombreProducto,
p.precioProducto,
fn_descuento_volumen(25,p.precioProducto) as TotalConDescuento # Se le pasa el precioProducto como parámetro
from producto p;

/*VISTAS
Definición: Consulta SQL temporal. Se almacena en la base de datos como vista, no como tabla.
Hacen las consultas más rápido. Solo muestran información. 

Sintaxis:

Si ya existe: CREATE OR REPLACE VIEW {nombre_vista} AS

CREATE VIEW {nombre_vista} AS 
SELECT {campos} FROM {tabla}
...;
Se puede usar toda la lógica de DML de consultas (order by, group by, joins, subqueries, etc.)
...
# Reto
# Clasificar los clientes según su historial de compras 
# Por cantidad de pedidos hechos (VIP, Normal, etc...)
select * from producto inner join categoria on idCategoriaFK = idCategoriajshfdjdk;

DELIMITER //
CREATE FUNCTION fn_clasificar_cliente(
	p_idCliente int
)
RETURNS VARCHAR(100)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_clasificacionCliente VARCHAR(100);
    DECLARE v_cantidadPedidos int;
    ...
    SET v_clasificacionCliente = case
        ...
        WHEN v_cantidadPedidos >= 3 THEN 'M.V.P'
...
COALESCE(avg(p.totalPedido),0) as PagoPromedio,
max(p.fechaPedido) as UltimoPedido,
fn_clasificar_cliente(c.idCliente) as Clasificacion
FROM cliente c
LEFT JOIN pedido p ON c.idCliente = p.idClienteFK
AND p.estadoPedido <> 'cancelado'
GROUP BY c.idCliente, c.nombreCliente, c.ciudadCliente, c.emailCliente;

select * from v_resumen_clientes order by MontoTotal desc;

select nombreCliente as Cliente, Clasificacion from v_resumen_clientes where ciudadCliente = 'Bogotá' order by MontoTotal desc;

/*
PARCIAL PRÁCTICO:
Nos va a dar tres tablas que están relacionadas. Nada cíclico. Nos va a dar el Script que crea la base de datos (puede tener errores), las tablas y algunas inserciones.
Cada uno va a tener un punto. Van a haber 16 puntos. "Crear una vista con una función"... "Crear un procedimiento con una vista que..." entra todo DDL, DML, TODO hasta vistas.
